...
%这里只是确定L曲线，而不是取得那个最优的lambda值，取最优在后面的一小段的l_corner(所以实际上还是求出了最优的lambda reg_corner)
%
...
npoints = 200;  % Number of points on the L-curve for Tikh and dsvd.        取多少合适
smin_ratio = 16*eps;  % Smallest regularization parameter.                  最小浮点数
...
if (strncmp(method,'Tikh',4) | strncmp(method,'tikh',4))                    %看字符串是否匹配
  ...
  marker = '-'; pos = .8; txt = 'Tikh.';                                    % 下面是其他方法 reg_param 就是lamda
...
if (locate)
  SkipCorner = ( (strncmp(method,'tsvd',4) | strncmp(method,'tgsv',4) | ...
                  strncmp(method,'mtsv',4)) & exist('spdemos')~=2 );
  if (SkipCorner)
    reg_corner = NaN;
  else
...
if (locate & ~SkipCorner)
  ...
  loglog([min(rho)/100,rho_c],[eta_c,eta_c],'--',...
         [rho_c,rho_c],[min(eta)/100,eta_c],'--')                          %%%%%% 在图中用虚线标出曲率最大的点
