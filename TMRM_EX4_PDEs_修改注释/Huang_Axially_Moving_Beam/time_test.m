...
q20=-0.05:0.001:0.05;
tic;
tt=0:0.01:600;odex=[q10(end);q20(end);0;0];
...
figure;
plot(num(50000:end,1),num(50000:end,3),'k-');
hold on
plot(num(50000:end,2),num(50000:end,4),'r-');
set(gca,'FontName','Times New Roman','FontSize',15,'LineWidth',1.5);
toc;
...
for i=1:length(q10)
    for j=1:length(q20)
        tic;
        tt=0:0.01:800;odex=[q10(i);q20(j);0;0];
        options=odeset('RelTol',1e-8,'AbsTol',1e-8);
        [t,num]=ode45('odehomo',tt,odex,options);
        if max(abs(num(70000:end,1)))>0.008
            index(i,j)=1;
        end
        i
        j
