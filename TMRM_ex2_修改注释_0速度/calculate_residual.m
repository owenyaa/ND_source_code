...
% figure;
% plot(time_data,acceleration_freq_sensitivity,'r-')
% hold on
% plot(time_data,ddx_w0,'b-')
% % velocity_freq_sensitivity=dx_w0;
% % acceleration_freq_sensitivity=ddx_w0;
% Compute the residual considering the frequency sensitivity
residual(num_degrees_freedom + 1:2 * num_degrees_freedom, :) = mass_matrix * acceleration_freq_sensitivity + damping_matrix * velocity_freq_sensitivity + stiffness_matrix * displacement_freq_sensitivity + 3 * [radius, 0; 0, beta] * (displacement.^2 .* displacement_freq_sensitivity);
...
for i = 1:2 * num_harmonics * num_degrees_freedom
    ...
    residual(num_degrees_freedom * (i + 1) + 1:num_degrees_freedom * (i + 2), :) = mass_matrix * acceleration_harm_sensitivity + damping_matrix * velocity_harm_sensitivity + stiffness_matrix * displacement_harm_sensitivity + 3 * [radius, 0; 0, beta] * (displacement.^2 .* displacement_harm_sensitivity);
end
